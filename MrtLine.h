...
class MrtLine
{
   ...
   public:
    ...
    // Thus now, I would need the cost from NS11 to NS12 (costPrev) and from NS12 to NS13
    // (costForward)
    ...
    void addNewStation(string stationId, string newStationName, bool infront, int costPrev,
                       int costForward);
    ...
    void updateConnBetweenStationsForAddNewStation(int selectedStationPtr, int oldStationIndex,
                                                   int costPrev, int costForward, Node* newStation,
                                                   int beforeAfter);
    ...
    Vector<string> getMrtStationNames();
